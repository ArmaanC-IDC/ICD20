...
pygame.init()
pygame.mixer.init()
ouch = pygame.mixer.Sound('unit 5/circle_catcher/beep.wav')
...
def create_circle():
    ...
    y = random.randint(0, HEIGHT - 2*radius)
...
while running:
    ...
    if keys[pygame.K_RIGHT]:
        player_x += player_speed
        player_x = min(player_x, WIDTH - player_radius)  # Ensure player does not move off the right edge
    if keys[pygame.K_UP]:
        player_y -= player_speed
        player_y = max(player_y, player_radius)  # Ensure player does not move off the top edge
    if keys[pygame.K_DOWN]:
        player_y += player_speed
        player_y = min(player_y, HEIGHT - player_radius)  # Ensure player does not move off the bottom edge
